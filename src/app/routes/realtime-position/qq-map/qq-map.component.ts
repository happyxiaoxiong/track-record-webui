...
@Component({
    selector: 'app-realtime-position-qq-map',
    template: `
        <full-content #target>
            ...
            <div nz-row [ngClass]="{'mt-sm': !fullScreen}">
                <div nz-col [nzMd]="rightClose ? 24 : 20" [nzSm]="rightClose ? 24 : 18" [nzXs]="rightClose ? 24 : 16">
                    <aqm-map fixWindow [minWidth]="0" (ready)="onReady($event)"
        ...
        if (!this.isOffLine(user)) {
            ...
            for (; i < this.onlineUsers.length; ++i) {
                ...
                if (user.userId === onlineUser.userId) {
                    ...
                    // 坐标转换
                    this.mapSrv.gps2MapLatLng(this.mapSrv.latLng(onlineUser)).subscribe((res) => {
                        onlineUser.position = res;
                        onlineUser.draw();
                        this.zone.run(() => {
            ...
            if (i === this.onlineUsers.length) {// 上线
                ...
                // 坐标转换
                this.mapSrv.gps2MapLatLng(this.mapSrv.latLng(newUser)).subscribe((res) => {
                    newUser.position = res;
                    this.zone.run(() => {
